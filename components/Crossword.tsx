...
import { CrosswordItem, CrosswordGrid, CrosswordClue } from '../types/crossword';
...
const Crossword: React.FC<CrosswordProps> = ({ data }) => {
      ...
      cells: grid.cells.map((row, rowIndex) =>
        row.map((cell, colIndex) =>
          rowIndex === y && colIndex === x
            ? { ...cell, input: value.toUpperCase(), correct: value.toUpperCase() === cell.letter }
            : cell
        )
      )
    };
  ...
  const findNextCell = (y: number, x: number): [number, number] | null => {
    ...
    const moveInDirection = (dy: number, dx: number): [number, number] | null => {
      let ny = y + dy;
      let nx = x + dx;
    ...
    const moveAcross = () => moveInDirection(0, 1) || moveInDirection(1, 0) || moveInDirection(0, -1) || moveInDirection(-1, 0);
    const moveDown = () => moveInDirection(1, 0) || moveInDirection(0, 1) || moveInDirection(0, -1) || moveInDirection(-1, 0);
  ...
  const handleCellClick = (y: number, x: number) => {
    ...
    const hasWordInCurrentDirection = (
      (currentDirection === 'across' && (x > 0 && grid!.cells[y][x - 1].letter || x < grid!.width - 1 && grid!.cells[y][x + 1].letter)) ||
      (currentDirection === 'down' && (y > 0 && grid!.cells[y - 1][x].letter || y < grid!.height - 1 && grid!.cells[y + 1][x].letter))
    );

    const hasWordInOppositeDirection = (
      (oppositeDirection === 'across' && (x > 0 && grid!.cells[y][x - 1].letter || x < grid!.width - 1 && grid!.cells[y][x + 1].letter)) ||
      (oppositeDirection === 'down' && (y > 0 && grid!.cells[y - 1][x].letter || y < grid!.height - 1 && grid!.cells[y + 1][x].letter))
    );
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-8 p-4 max-w-7xl mx-auto">
      <div className="w-full lg:w-2/3">
        <div className="grid gap-1 aspect-square" style={{ gridTemplateColumns: `repeat(${grid.width}, minmax(0, 1fr))` }}>
          {grid.cells.map((row, y) =>
            row.map((cell, x) => (
              <div
                key={`${x}-${y}`}
                className={`aspect-square border ${cell.letter ? 'bg-white' : 'bg-gray-200'} flex items-center justify-center relative`}
                onClick={() => handleCellClick(y, x)}
              >
                {cell.number && (
                  <span className="absolute top-0 left-0 text-xs sm:text-sm">{cell.number}</span>
                )}
                {cell.letter && (
                  <input
                    ref={(el) => {
                    ...
                    className={`w-full h-full text-center text-base sm:text-lg md:text-xl lg:text-2xl font-bold focus:outline-none focus:ring-2 focus:ring-blue-500
                      ${cell.correct === true ? 'bg-green-200' : cell.correct === false ? 'bg-red-200' : ''}`}
        ...
        <div className={`mt-4 p-2 text-center text-lg font-bold ${allCorrect ? 'bg-green-500 text-white' : 'bg-yellow-500 text-white'}`}>
          {allCorrect ? 'Congratulations! All answers are correct!' : 'Keep going! You can do it!'}
        </div>
      </div>
